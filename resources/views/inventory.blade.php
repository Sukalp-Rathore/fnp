                        ...
                        <tbody>
                            @foreach ($products as $p)
                                <tr>
                                    ...
                                    <td>{{ $p->quantity ?? '' }}</td>
                                    <td><button class="btn btn-secondary btn-sm viewProductImageBtn" data-id="{{$p->_id}}">View Image</button></td>
                                    <td>
                                        <button class="btn btn-primary btn-sm editBtn" data-id="{{$p->_id}}">Edit</button>
            ...
            <div class="modal-body" id="response">
                <form id="addProductForm" method="POST" action="{{route('inventory.create')}}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="product_name" name="product_name" placeholder="Enter Product Name" required autocomplete="off">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="cost_price" name="cost_price" placeholder="Enter Cost Price" autocomplete="off">
                    </div>
                    <div class="mb-3">
                        <label for="selling_price" class="form-label">Selling Price</label>
                        <input type="number" class="form-control" id="selling_price" placeholder="Enter Selling Price" name="selling_price" autocomplete="off">
                    </div>
                    <div class="mb-3">
                        <label for="product_image" class="form-label">Add Image</label>
                        <input type="file" accept="image/*" class="form-control" id="product_image" name="product_image" autocomplete="off">
                    </div>
                    <div class="mb-3">
...
<div class="modal fade" id="editInventoryModal" tabindex="-1" aria-labelledby="exampleModalLgLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
...
<div class="modal fade" id="updateStockModal" tabindex="-1" aria-labelledby="exampleModalLgLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
...
$(document).ready(function() {
    ...
    $(document).on('click', '.update-stock', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#stockTableContainer').html(response); // Load the table into the modal
            },
            error: function(xhr, status, error) {
                console.error(xhr.responseText);
    ...
    $(document).on('click', '#saveStockChanges', function() {
        ...
        $('#stockTableContainer').find('tr').each(function() {
            ...
            if (id && quantity) {
                stockData.push({ id: id, quantity: parseInt(quantity) });
            }
        });
